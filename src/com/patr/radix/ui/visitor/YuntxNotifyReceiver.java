...
public class YuntxNotifyReceiver extends ECNotifyReceiver {
   ...
   /* @Override
    ...
    public void showToast(ECMultiDeviceState deviceState){
        ...
        ToastUtil.showShort(App.instance.getApplicationContext(), stringBuilder.toString());
    ...
    public void onNotificationClicked(Context context, int notifyType, String sender) {
        LogUtil.d("onNotificationClicked notifyType " + notifyType + " ,sender " + sender);
    ...
    public static class NotifyService extends Service {
        ...
        private void receiveImp(Intent intent) {
            if(intent == null) {
                LogUtil.e("receiveImp receiveIntent == null");
                return ;
            }
            LogUtil.i("intent:action " + intent.getAction());
            int optCode = intent.getIntExtra(SERVICE_OPT_CODE, 0);
            if(optCode == 0) {
                LogUtil.e("receiveImp invalid opcode.");
                return ;
            }
        ...
        public int onStartCommand(Intent intent, int flags, int startId) {
            LogUtil.v(String.format("onStartCommand flags :%d startId :%d intent %s", flags, startId, intent));
            receiveImp(intent);
